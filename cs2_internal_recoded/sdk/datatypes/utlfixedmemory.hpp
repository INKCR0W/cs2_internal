...
public:
	...
	void Grow(const int nCount = 1)
	{
		...
		BlockHeader_t* pNewBlockHeader = static_cast<BlockHeader_t*>(interfaces::mem_alloc->alloc(sizeof(BlockHeader_t) + nBlockSize * sizeof(T)));
		ASSERT(pNewBlockHeader != nullptr); // container overflow
	...
	void Purge()
	{
		...
		for (BlockHeader_t* pBlockHeader = pBlocks; pBlockHeader != nullptr;)
		{
			...
			pBlockHeader = pBlockHeader->pNext;
			interfaces::mem_alloc->free(pFree);
		}
