...
#define CLIP_TRACE_TO_PLAYERS xorstr_("48 8B C4 55 56 48 8D A8 58 FF FF FF 48 81 EC 98 01 00 00 48")
#define TRACE_SHAPE xorstr_("48 89 54 24 ? 48 89 4C 24 ? 55 53 56 57 41 55 41 56 48 8D AC 24")
...
public:
	void InitializeTraceInfo(Game_Trace_t* const Hit)
	{
		...
		if (!InitializeTraceInfoFn)
			InitializeTraceInfoFn = reinterpret_cast<InitializeTraceInfoFunction>(memory::mem.find_pattern(modules::client_dll, xorstr("48 89 5C 24 08 57 48 83 EC 20 48 8B D9 33 FF 48 8B 0D")));

		FakeReturnAddress(hook::return_address, InitializeTraceInfoFn, Hit);
	}
	...
	void InitializeTrace(Game_Trace_t& Trace)
	{
		...
		if (!InitializeTraceFn)
			InitializeTraceFn = reinterpret_cast<InitializeTraceFunction>(memory::mem.find_pattern(modules::client_dll, xorstr("48 89 5C 24 08 57 48 83 EC 20 48 8B D9 33 FF 48 8B 0D")));

		FakeReturnAddress<void, Game_Trace_t&>(hook::return_address, InitializeTraceFn, Trace);
	}

	void Init(Trace_Filter_t& Filter, entity::C_CSPlayerPawn* Skip, uint64_t Mask, uint8_t Layer, uint16_t Idk)
	{
		...
		if (InitTraceFilterFn == nullptr)
			InitTraceFilterFn = reinterpret_cast<InitTraceFilterFunction>(memory::mem.find_pattern(modules::client_dll, xorstr_("48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 20 0F B6 41 37 33")));

		FakeReturnAddress<Trace_Filter_t*, Trace_Filter_t&, void*, uint64_t, uint8_t, uint16_t>(hook::return_address, InitTraceFilterFn, Filter, Skip, Mask, Layer, Idk);
	}
	...
	void ClipTraceToPlayers(Vector_t& Start, Vector_t& End, Trace_Filter_t* Filter, Game_Trace_t* Trace, float Min, int Length, float Max)
	{
		...
		{
			ClipTraceToPlayersFn = reinterpret_cast<ClipTraceToPlayersFunction>(memory::mem.find_pattern(modules::client_dll, CLIP_TRACE_TO_PLAYERS));
			if (ClipTraceToPlayersFn == nullptr)
				ClipTraceToPlayersFn = reinterpret_cast<ClipTraceToPlayersFunction>(memory::mem.find_pattern(modules::client_dll, xorstr("48 8B C4 55 56 48 8D A8 58")));
		}

		FakeReturnAddress<void, Vector_t&, Vector_t&, Trace_Filter_t*, Game_Trace_t*, float, int, float>(hook::return_address, ClipTraceToPlayersFn, Start, End, Filter, Trace, Min, Length, Max);
	}
	...
	static void GetTraceInfo(Trace_Data_t* Trace, Game_Trace_t* Hit, const float UnknownFloat, void* Unknown)
	{
		...
		if (GetTraceInfoFn == nullptr)
			GetTraceInfoFn = reinterpret_cast<GetTraceInfoFunction>(memory::mem.find_pattern(modules::client_dll, xorstr_("48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 48 83 EC 60 48 8B E9 0F")));

		FakeReturnAddress<void, Trace_Data_t*, Game_Trace_t*, float, void*>(hook::return_address, GetTraceInfoFn, Trace, Hit, UnknownFloat, Unknown);
	}
	...
	static bool HandleBulletPenetration(Trace_Data_t* const Trace, void* Stats, UpdateValueT* const ModValue, const bool draw_showimpacts = false)
	{
		...
		if (HandleBulletPenetrationFn == nullptr)
			HandleBulletPenetrationFn = reinterpret_cast<HandleBulletPenetrationFunction>(memory::mem.find_pattern(modules::client_dll, xorstr_("48 8B C4 44 89 48 20 55 57 41 55")));

		return FakeReturnAddress<bool, Trace_Data_t*, void*, UpdateValueT*, void*, void*, void*, void*, void*, bool>(hook::return_address, HandleBulletPenetrationFn, Trace, Stats, ModValue, nullptr, nullptr, nullptr, nullptr, nullptr, draw_showimpacts);
	}
	...
	static void CreateTrace(Trace_Data_t* const Trace, const Vector_t Start, const Vector_t End, const Trace_Filter_t& Filter, const int PenetrationCount)
	{
		...
		if (CreateTraceFn == nullptr)
			CreateTraceFn = reinterpret_cast<CreateTraceFunction>(memory::mem.find_pattern(modules::client_dll, xorstr_("48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 41 56 41 57 48 83 EC 40 F2")));

		FakeReturnAddress<void, Trace_Data_t*, Vector_t, Vector_t, Trace_Filter_t, int>(hook::return_address, CreateTraceFn, Trace, Start, End, Filter, PenetrationCount);
	}
	...
	void TraceShape(Ray_t& Ray, Vector_t* Start, Vector_t* End, Trace_Filter_t Filter, Game_Trace_t& Trace)
	{
		...
		if (TraceShapeFn == nullptr)
			TraceShapeFn = reinterpret_cast<TraceShapeFunction>(memory::mem.find_pattern(modules::client_dll, xorstr_("48 89 5C 24 10 48 89 74 24 18 48 89 7C 24 20 48 89 4C 24 08 55 41 54 41 55 41 56 41 57 48 8D")));

		FakeReturnAddress<bool, void*, Ray_t&, Vector_t*, Vector_t*, Trace_Filter_t, Game_Trace_t&>(hook::return_address, TraceShapeFn, this, Ray, Start, End, Filter, Trace);
	}
};
