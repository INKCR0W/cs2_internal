...
// used: cutllinkedlist
#include "../datatypes/utllinkedlist.hpp"
...
public:
	...
	CConVar* find(const char* CVarName)
	{
		for (int i = interfaces::cvar->listConvars.Head(); i != interfaces::cvar->listConvars.InvalidIndex(); i = interfaces::cvar->listConvars.Next(i))
		{
			CConVar* pConVar = interfaces::cvar->listConvars.Element(i);
			if (pConVar == nullptr)
				continue;

			if (crt::crt.string_compare(pConVar->szName, CVarName))
				return pConVar;
		}
